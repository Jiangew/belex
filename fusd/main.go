...
func sendMessage(api exchange.API, bot *tgbot.BotAPI, updates tgbot.UpdatesChannel) {
	for update := range updates {
		...
		case "start":
			maxBuyPrice = float64(0)
			minSellPrice = float64(0)
			msg := tgbot.NewMessage(update.Message.Chat.ID, "started...\n"+"max buy and min sell limit price in memory has been cleared.")
			msg.ReplyToMessageID = update.Message.MessageID
			_, _ = bot.Send(msg)
		case "stop":
			ticker, _ := api.GetTicker(symbol)
			...
			msg := tgbot.NewMessage(update.Message.Chat.ID, "stopped...\n"+"max buy and min sell limit price in memory has been set.")
			msg.ReplyToMessageID = update.Message.MessageID
			_, _ = bot.Send(msg)
		}
	}
